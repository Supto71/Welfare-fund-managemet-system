...
export default function WelfareFundPage() {
  ...
  // Search Summary: calculate totals for current filtered results
  const searchSummary = filteredTransactions.reduce(
    (acc, t) => {
      if (t.type === 'donation') acc.totalDonation += Number(t.amount) || 0
      else acc.totalExpense += Number(t.amount) || 0
      return acc
    },
    { totalDonation: 0, totalExpense: 0 }
  )
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col">
      ...
      <main id="printable-full-report" className="flex-1 w-full max-w-6xl mx-auto p-4 sm:p-6 lg:p-8 animate-fade-in space-y-6">
        ...
        {isSearchActive && (
          <div className="bg-gradient-to-r from-brand-navy/5 to-brand-gold/10 border border-brand-navy/20 rounded-2xl px-5 py-4 flex flex-col sm:flex-row sm:items-center gap-3 sm:gap-6 no-print animate-fade-in">
