...
export default function StatCard({ label, sublabel, value, icon, color = 'green', prefix = null, suffix = '', isCurrency = true }) {
  ...
  return (
    <div className={`${c.bg} rounded p-1 shadow-sm border border-white hover:shadow-md transition-shadow duration-200 flex items-center gap-1.5`}>
      <div className={`${c.icon} w-3.5 h-3.5 rounded flex items-center justify-center shrink-0 ring-1 ${c.ring}`}>
        {icon}
      </div>
      <div className="min-w-0">
        <p className="text-gray-500 text-[9px] font-semibold leading-none truncate">{label}</p>
        <div className={`${c.val} text-sm font-extrabold mt-px tracking-tight flex items-center gap-0.5`}>
          {prefix}
